...
UnityLight CreateLight(float3 normal, fixed detailShadowMap)
{
    ...
    light.color = saturate(_LightColor0.rgb * lerp(1, poiLight.attenuation, float(0.619)) * detailShadowMap);
...
float FadeShadows(float attenuation)
{
            ...
            attenuation = lerp(1, poiLight.attenuation, float(0.619));
        #endif
...
void ApplySubtractiveLighting(inout UnityIndirect indirectLight)
{
        ...
        poiLight.attenuation = FadeShadows(lerp(1, poiLight.attenuation, float(0.619)));
...
UnityIndirect CreateIndirectLight(float3 normal)
{
    ...
    indirectLight.specular = 0;
        ...
        envData.roughness = 1 - float(0.101);
...
{
    return UNITY_BRDF_PBS(1, 0, 0, float(0.101), poiMesh.normals[1], poiCam.viewDir, CreateLight(poiMesh.normals[1], detailShadowMap), CreateIndirectLight(poiMesh.normals[1])).xyz;
}
void calculateBasePassLightMaps()
{
        ...
        float bw_directLighting = (((poiLight.nDotL * 0.5 + 0.5) * bw_lightColor * lerp(1, poiLight.attenuation, float(0.619))) + dot(ShadeSH9Normal(poiMesh.normals[1]), grayscale_vector));
        ...
        if (float(1) == 2) // Wrapped
        {
            ...
            float directatten = lerp(1, poiLight.attenuation, float(0.619));
